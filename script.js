...
for(let i=1;i<=8;i++){
    ...
    audio.volume=0.5;
    audio.addEventListener('ended',()=>{currentTrack=(currentTrack+1)%tracks.length; playMusic();});
    tracks.push(audio);
}
function playMusic(){tracks.forEach(t=>{t.pause();t.currentTime=0;}); tracks[currentTrack].play().catch(()=>{});}
...
async function loadQuestions(){
    ...
    for(const f of files){
        ...
        all.push(...data); // preserve JSON order
    }
    return all;
}
...
/* TIMER */
function startTimer(){timerInterval=setInterval(()=>{timerEl.textContent=((Date.now()-startTime)/1000).toFixed(1)+'s';},100);}
...
function renderMini(q){
    ...
    const makeBtn=(text,handler)=>{const b=document.createElement('button'); b.textContent=text; b.onclick=handler; answersDiv.appendChild(b); return b;}
    ...
    if(q.mini==='avoid'){
        for(let i=1;i<=q.buttons;i++) makeBtn('Button '+i,()=> i===q.safe?(flash('correct'),next()):loseLife());
    }
    ...
    if(q.mini==='avoid-move'){
        for(let i=1;i<=q.buttons;i++){
            ...
            btn.style.position='absolute';
            const move=()=>{btn.style.left=Math.random()*(answersDiv.clientWidth-100)+'px'; btn.style.top=Math.random()*(answersDiv.clientHeight-50)+'px';};
            setInterval(move,150); move();
        }
    }
}

/* NEXT */
function next(){index++; if(index>=questions.length) finishQuiz(); else showQuestion();}
...
function saveScore(user,time){
    ...
    let name=user; let dup=lb.filter(e=>e.username.startsWith(user)).length;
